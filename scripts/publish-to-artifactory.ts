...
for (const demoDir of demoDirs) {
    ...
    try {
        ...
        execSync(
            `tar -czf ${tarPath} -C ${demoDir} --exclude=node_modules --exclude=dist --exclude=package-lock.json .`,
        );
    ...
    try {
        ...
        execSync(
            `gh release create ${tagName} ${tarPath} --title "Release ${tagName}" --notes "Automated demo release for ${demoName} version ${version}"`,
        );
