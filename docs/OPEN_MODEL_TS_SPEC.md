...
## 3. Model Declaration Comparison

### 3.1 Legacy Approach (Manual Wiring)

The previous approach required explicit string names and manual dependency injection, leading to verbose boilerplate and
potential type-safety loss.
...
### 3.2 Modern Declarative Approach (Decorators)

The new specification utilizes class getters and decorators. Dependencies are auto-discovered through standard `this.`
property access.
...
To support this specification, `src/bindings.ts` will need to:

1. Export the required decorator functions (`@Workbook`, `@Input`, `@Node`, `@Chart`, `@Table`) utilizing TS 5.0
   `ClassGetterDecoratorContext` and `ClassAccessorDecoratorContext`.
2. Adapt the `executeWithTracking` function to accept the `this` context and dynamically resolve property names using
   `context.name`.
3. Ensure the `evalWorkbook` and `mutateInput` host-bridge functions can interact seamlessly with instantiated Workbook
   classes.
...
The transition to the modern declarative approach will be executed in three distinct phases to ensure stability and
maintain backward compatibility during the refactoring.

### Phase 1: Framework Enhancement (`src/bindings.ts`)

- [ ] **Core Decorators:** Implement and export `@Workbook`, `@Node`, `@Chart`, `@Table`, and `@Input`.
- [ ] **Context Awareness:** Update `executeWithTracking` to handle class instance contexts and property-based
  identification.
- [ ] **Class Lifecycle:** Update `evalWorkbook` to support both legacy thunk-based loaders and modern class
  constructors.
- [ ] **Mutation Bridge:** Update `mutateInput` to correctly target class `accessor` properties.

### Phase 2: Engine & Tooling Verification

- [ ] **Transpilation Audit:** Verify that `OpenModelTSEngine` correctly sanitizes and bundles the new `__esDecorate`
  and `__runInitializers` helpers generated by TypeScript.
- [ ] **Unit Testing:** Create a new test suite `tests/decorator_test.ts` to verify the new syntax in isolation.
- [ ] **Regression Testing:** Run existing tests (`bindings_test.ts`, `engine_test.ts`) to ensure zero regressions in
  the core reactivity engine.

### Phase 3: Demo Migration & Validation

- [ ] **Loan Schedule Demo:** Refactor `demo/loan-schedule/main.ts` to use the new class-based decorator syntax.
- [ ] **Loan Originations Demo:** Refactor `demo/loan-originations/main.ts` to use the new class-based decorator syntax.
