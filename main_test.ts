...
import {clearTrace, defineWorkbook, getFromTrace, trace} from "./main.ts";
...
Deno.test("Pull execution: leaf node triggers upstream chain", () => {
  ...
  assertEquals(getFromTrace("calculateMonthlyPayment"), undefined);
  assertEquals(getFromTrace("generateLoanSchedule"), undefined);

  // Trigger pull
  workbook.renderLoanBalanceChart();

  // Verify upstream nodes were executed and traced
  const monthlyPaymentResult = getFromTrace("calculateMonthlyPayment");
  assertExists(monthlyPaymentResult);
  assertEquals(typeof monthlyPaymentResult.monthlyPayment, "number");

  const scheduleResult = getFromTrace("generateLoanSchedule");
...
Deno.test("Mapping: calculateMonthlyPayment uses correct inputs", () => {
  ...
  // Check inputs recorded in trace
  ...
  const result = getFromTrace("calculateMonthlyPayment");
...
Deno.test("Memoization: subsequent calls use trace", () => {
  ...
  trace("calculateMonthlyPayment", mockResult);
